...
import { ArrowLeft, Save, RotateCcw, Database, Eye, EyeOff, FileText, Loader2, Edit2 } from 'lucide-react';
...
export default function EditIndikator() {
    ...
    const fetchDetail = async () => {
      ...
      try {
        ...
        if (docSnap.exists()) {
          ...
          // Parse formula
          try {
            const parsed = JSON.parse(data.formula || '{}');
            if (parsed.numerator !== undefined && parsed.denominator !== undefined) {
              setFormulaObj({ numerator: parsed.numerator, denominator: parsed.denominator });
            } else {
              const stripped = data.formula ? data.formula.replace(/<[^>]*>?/gm, '') : '';
              setFormulaObj({ numerator: stripped, denominator: '' });
            }
          } catch (e) {
            const stripped = data.formula ? data.formula.replace(/<[^>]*>?/gm, '') : '';
            setFormulaObj({ numerator: stripped, denominator: '' });
  ...
  const handleReset = () => {
    if (window.confirm('Apakah Anda yakin ingin mereset perubahan?')) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(`/profil-indikator/${id}`);
    } catch (error) {
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-gray-600 dark:text-[var(--muted-foreground)] mt-1 text-sm">Perbarui profil indikator standar Kemenkes / SNARS</p>
            </div>
          </div>
